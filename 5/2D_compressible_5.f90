...
program navierstokes
    ...
    !BEGINNING OF TIME LOOP
      do n=1,nt
         if (itemp.eq.1) then   !TEMPORAL SCHEME AB2
            ...
            call boundary(rho,rou,rov,roe,scp,nx,ny,dlx,dlt,uu0,chp,gma)
    ...
    subroutine boundary(rho,rou,rov,roe,scp,nx,ny,dlx,dlt,uu0,chp,gma)
    !
    !Enforcing the inlet and outlet boundary conditions
    !#############################################
    !
      implicit none
    !
      real(8),dimension(nx,ny) :: rho,rou,rov,roe,scp
      real(8),dimension(ny) :: dro,dru,drv,dre,dtp
      real(8) :: roi,tpi,uu0,vv0,chv,chp,gma,udx,dlx,dlt
      integer :: nx,ny,j

      !Enforcing the inlet BC by setting all quantities to same as initial state
      !
      !Assigning some initial states (initial v is assigned 0 for simplicity, ignoring noise)
      !roi: initial value for density
      !tpi: initial value for temperature
      !vv0: initial value for v-velocity
      ...
      do j=1,ny
         ...
         rou(1,j)=roi*uu0
         rov(1,j)=roi*vv0
         roe(1,j)=chv*tpi+0.5*(uu0*uu0+vv0*vv0)
         scp(1,j)=1.
      enddo

    !Outlet BC
    !
    !First, calculate the spatial derivatives using an upstream first order scheme,
    !and multiply derivatives by -U_c to get fluxes as per the linear advection equation.
      udx=1./dlx
      do j=1,ny
         dro(j)=-uu0*udx*(rho(nx,j)-rho(nx-1,j))
         dru(j)=-uu0*udx*(rou(nx,j)-rou(nx-1,j))
         drv(j)=-uu0*udx*(rov(nx,j)-rov(nx-1,j))
         dre(j)=-uu0*udx*(roe(nx,j)-roe(nx-1,j))
         dtp(j)=-uu0*udx*(scp(nx,j)-scp(nx-1,j))
      enddo

    !Then advance time using an explicit euler method
      do j=1,ny
         rho(nx,j)=rho(nx,j)+dlt*dro(j)
         rou(nx,j)=rou(nx,j)+dlt*dru(j)
         rov(nx,j)=rov(nx,j)+dlt*drv(j)
         roe(nx,j)=roe(nx,j)+dlt*dre(j)
         scp(nx,j)=scp(nx,j)+dlt*dtp(j)
      enddo
    ...
    !############################################
      ...
      do j=1,ny
         ...
         enddo
         dfi(nx,j)=udx*(phi(nx-2,j)-4.*phi(nx-1,j)+3.*phi(nx,j))
      enddo
      !Implemented second order forward and backwards schemes for the boundaries
      ...
      do j=1,ny
         ...
         do i=2,nx-1
            dfi(i,j)=udx*(phi(i+1,j)-(phi(i,j)+phi(i,j))&
                 +phi(i-1,j))
         enddo
         dfi(nx,j)=udx*(-phi(nx-3,j)+4.*phi(nx-2,j)-5.*phi(nx-1,j)+2.*phi(nx,j))
      enddo
      !Implemented second order forward and backwards schemes for the boundaries
      return
    ...
    !######for the square cylinder########################################
          ic=nint((xlx/4./dlx)+1) !X coordinate center of square
